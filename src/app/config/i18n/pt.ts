// Brasil
export const locale = {
	lang: 'pt',
	data: {
		TRANSLATOR: {
			SELECT: 'Selecione a Lingua',
		},
		MENU: {
			DASHBOARD: 'Dashboard',
			INGREDIENTS: 'Ingredientes',
			RECIPE: 'Receitas',
			MENU: 'Cardápios',
		},
		INPUTS: {
			REGION: 'Estado',
			FILTERBYSTATE: 'Filtrar Ingredientes por Estado',
			OPTION_DEFAULT: 'Selecione uma opção',
			SAVE: 'Salvar',
			BACK: 'Voltar',
			NEW: 'Novo',
			NEW_STEP: 'Novo passo',
			AMOUNT: 'Quantidade',
			PRICE: 'Preço',
			GROSSWEIGHT: 'Peso Bruto',
			NETWEIGHT: 'Peso Limpo'
		},
		INGREDIENT: {
			TITLE: 'Cadastro de Ingrediente',
			TITLE_PURCHASE_PRICE: 'Preço de Compra',
			NO_RESULTS: 'Sem ingredientes cadastrados.',
			INPUT: {
				NAME: 'Nome',
				CATEGORY: 'Categoria',
				UNIT: 'Medida',
				DESCRIPTION: 'Descrição',
				PRICE: 'Preço',
				UNITQUANTITY: 'Quantidade'
			}
		},
		RECIPE: {
			TITLE: 'Cadastro de Receita',
			NO_RESULTS: 'Sem receitas cadastradas.',
			INPUT: {
				NAME: 'Nome',
				CATEGORY: 'Categoria',
				PREPARATION_TIME: 'Tempo de preparo',
				UNIT: 'Medida',
				DESCRIPTION: 'Descrição',
				UNITQUANTITY: 'Rendimento',
				SELL_VALUE: 'Valor de Venda',
				PROFIT_PERC: 'Margem de Lucro'
			},
			TABS: {
				GENERAL: { TITLE: 'Geral' },
				INGREDIENTS: {
					TITLE: 'Ingredientes'
				},
				PREPARATION: {
					TITLE: 'Preparo',
					METHOD_OF_PREPARATION: 'Modo de Preparo',
				},
				FINANCIAL: {
					TITLE: 'Financeiro',
					TOTAL_COAST: 'Custo Total {{ cost }}',
					UNIT_COAST: 'Custo Unidade'
				}
			}
		},
		MENUS: {
			NO_RESULTS: 'Sem cardápios cadastrados.',
		},
		PROFILE: {
			TITLE: 'Seu perfil',
			USER: {
				DETAIL: 'Detalhes Pessoais',
				NAME: 'Nome Completo',
				EMAIL: 'Email'
			},
			AMOUNT: {
				INGREDIENT: 'Ingredientes',
				RECIPE: 'Receitas',
				MENU: 'Cardápios'

			},
			BTN_SAVE_CHANGES: 'Salvar Alterações',
			BTN_CANCEL: 'Cancelar'
		},
		AUTH: {
			GENERAL: {
				OR: 'Ou',
				NO_ACCOUNT: 'Ainda não tem conta?',
				SIGNUP_BUTTON: 'Registrar',
				FORGOT_BUTTON: 'Esqueceu a senha',
				BACK_BUTTON: 'Voltar',
				PRIVACY: 'Privacidade',
				LEGAL: 'Legal',
				CONTACT: 'Contato',
				RECOVER: 'Recuperar'
			},
			LOGIN: {
				TITLE: 'Bem vindo ao Chefpro',
				BUTTON: 'Entrar',
			},
			FORGOT: {
				TITLE: 'Esqueceu a senha?',
				DESC: 'Enter com seu email para resetar sua senha',
			},
			REGISTER: {
				TITLE: 'Registrar',
				DESC: 'Entre com suas informações para a criação da conta'
			},
			INPUT: {
				EMAIL: 'Email',
				NAME: 'Nome',
				PASSWORD: 'Senha',
				AGREE: 'Eu aceito os',
				AGREE_TERMS: 'termos e condições',
			}
		},
		VALIDATION: {
			INVALID: '{{name}} não é válido',
			REQUIRED: '{{name}} é requerido',
			MIN_LENGTH: '{{name}} tamanho minimo é {{min}}',
			AGREEMENT_REQUIRED: 'Os Termos e condições é obrigatório'
		}
	}
};
